...
upd_pa :: Show state => Ord state =>
          EpistM state -> Form state -> EpistM state
upd_pa m@(Mo states agents val rels actual) f = Mo states' agents val' rels' actual'
   where 
...
upd_pc :: Show state => Ord state => [Prp] -> EpistM state
                    -> [(Prp,Form state)] -> EpistM state
upd_pc ps m@(Mo states agents _ rels actual) sb = 
  Mo states agents val' rels actual
   where 
   val' = [ (s, [p | p <- ps, isTrueAt m s (sub sb p)]) 
                                          | s <- states ]

upds_pc :: Show state => Ord state => [Prp] -> EpistM state
                     -> [[(Prp,Form state)]] -> EpistM state
upds_pc ps = foldl (upd_pc ps)

upd_pi :: (state -> state) -> EpistM state -> EpistM state
upd_pi f (Mo states agents val rels actual) =
  Mo 
  (map f states)
  agents
  (map (first f) val)
