...
singleTestWith f = do
  let a = SMCDEL.Explicit.Simple.eval mymodel f             -- evaluate directly on Kripke
  let b = SMCDEL.Symbolic.HasCacBDD.eval myscn f                  -- evaluate directly on KNS
  let c = SMCDEL.Symbolic.HasCacBDD.evalViaBdd myscn f            -- evaluate boolean equivalent on KNS
  let d = SMCDEL.Explicit.Simple.eval (knsToKripke myscn) f -- evaluate on corresponding Kripke
  let e = SMCDEL.Symbolic.HasCacBDD.eval  (kripkeToKns mymodel) f -- evaluate on corresponding KNS
...
pubAnnounceTest = do
  ...
  let a = SMCDEL.Explicit.Simple.eval mymodel (PubAnnounce f g)
  putStr $ show a
  let b = SMCDEL.Symbolic.HasCacBDD.eval (kripkeToKns mymodel) (PubAnnounce f g)
  putStr $ show b
  let c = SMCDEL.Symbolic.HasCacBDD.eval (knowTransform (kripkeToKns mymodel) (actionToEvent (pubAnnounceAction ["0","1","2"] f))) g
  print c
  if a/=b || b/=c
...
singleActionTest = do
  ...
  f <- getRandomForm 3
  let a = SMCDEL.Explicit.Simple.eval (productUpdate mymodel myact) f
  let b = SMCDEL.Symbolic.HasCacBDD.evalViaBdd (knowTransform (kripkeToKns mymodel) (actionToEvent myact)) f
  if a /= b
