...
-- We use longer and longer formulas for the rounds.
...
-- | Generic function to solve the puzzle, keeping the intermediate models.
-- This version keeps the intermediate models instead of making longer formulas.
findNumberWithKeep :: (Int -> Int -> a, a -> Form -> Bool, a -> Form -> a) -> Int -> Int -> Int
findNumberWithKeep (start, evalfunction, announcefunction) n m = k where
  afterF = announcefunction (start n m) (father n)
  k | loop 0 afterF == (m-1) = m-1
    | otherwise       = error $ "wrong Muddy Children result: " ++ show (loop 0 afterF :: Int)
  loop count model = if evalfunction model (nobodyknows n)
    then loop (count+1) (announcefunction model (nobodyknows n))
    else count
...
findNumberKeepCacBDD :: Int -> Int -> Int
findNumberKeepCacBDD = findNumberWithKeep (cacMudScnInit,SMCDEL.Symbolic.S5.evalViaBdd, update) where
  cacMudScnInit n m = ( SMCDEL.Symbolic.S5.KnS (mudPs n) (SMCDEL.Symbolic.S5.boolBddOf Top) [ (show i,delete (P i) (mudPs n)) | i <- [1..n] ], mudPs m )
...
benchMain = do
  ...
  defaultMainWith myConfig (map mybench
    ...
    , ("CacBDDkeep", findNumberKeepCacBDD, [3..40] )
